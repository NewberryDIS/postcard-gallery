...
const css = {
  code: ".tag-button.svelte-1kedgcl{margin-inline:16px;background:rgb(var(--bg-color-1))}.right.svelte-1kedgcl{display:flex;flex-direction:column}",
  map: null
};
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.data === void 0 && $$bindings.data && data !== void 0)
    $$bindings.data(data);
  $$result.css.add(css);
  return `${$$result.head += `<!-- HEAD_svelte-1blp97s_START -->${$$result.title = `<title>Newberry Postcard Galleries</title>`, ""}<!-- HEAD_svelte-1blp97s_END -->`, ""} <main class="main">${validate_component(Secret_header, "SecretHeader").$$render($$result, { title: "Newberry Postcard Gallery" }, {}, {})} <div class="left"><div class="logo" data-svelte-h="svelte-8chl25"> <a href="${escape(base, true) + "/"}"><img class="biggo no-share" src="${escape(base, true) + "/NewberryLogo.png"}" alt="Logo for the Newberry Library" height="60" width="300"> <img class="smallo no-share" src="${escape(base, true) + "/NLogo.png"}" alt="Logo for the Newberry Library" height="60" width="60"></a></div> <header class="header"><section class="title" data-svelte-h="svelte-kjtxng"><h1 class="">Newberry Postcard Gallery</h1></section> <aside class="text-content"><p class="text-lg">The vintage postcard images featured here are free to use and reuse. The Newberry believes
					...
					.</p> <p class="text-base">The Newberry houses one of the largest public collections of postcards and related
					materials in the United States. Learn more at our${escape(" ")} <a href="https://www.newberry.org/collection/subjects/postcards" target="_blank" class="llines" data-svelte-h="svelte-z17dku">Postcards information page</a>
					, browse more than${escape(" ")} <a href="https://collections.newberry.org/asset-management?WS=SearchResults#/DamView&VBID=2KXJA4UET6ZH&PN=1&WS=SearchResults" target="_blank" class="llines" data-svelte-h="svelte-odtp07">50,000 digitized postcards
					</a>${escape(" ")}
					at Newberry Digital Collections, or explore the themed sets featured here.</p> <p class="text-sm">With gratitude to the Library of Congress for its${escape(" ")} <a href="https://www.loc.gov/free-to-use/" target="_blank" class="llines" data-svelte-h="svelte-1n83kqe">Free to Use and Reuse Sets</a>
					, from which this site is inspired.</p></aside></header> ${validate_component(Footer, "Footer").$$render($$result, {}, {}, {
    default: () => {
      return `<section class="tag-button svelte-1kedgcl" data-svelte-h="svelte-6u9cli"><p>The Newberry needs your help! Please assist with making our postcard collections more
					accessible at
    ...
    default: () => {
      return `${each(nhGalleries, (item) => {
        return `<a${add_attribute("href", item.link || `${base}/${item.slug}`, 0)}${add_attribute("class", item.pixel ? "noimg" : "", 0)}${add_attribute("target", item.pixel ? "_blank" : "_self", 0)}><img class="no-share"${add_attribute(
          ...
          0
        )} alt="${"a " + escape(item.title, true) + " postcard"}"${add_attribute("height", getHeight(item.width, item.height), 0)} width="300"> <h3>${escape(item.title)}</h3> </a>`;
      })}`;
    }
  })} ${validate_component(Masonry, "Masonry").$$render($$result, {}, {}, {
    default: () => {
      return `${each(hGalleries, (item) => {
        return `<a class="holiday" href="${escape(base, true) + "/" + escape(item.slug, true)}"><img class="no-share" src="${"https://collections.newberry.org/IIIF3/Image/" + escape(item.image, true) + "/full/300,/0/default.jpg"}" alt="${"a " + escape(item.title, true) + " postcard"}"${add_attribute("height", getHeight(item.width, item.height), 0)} width="300"> <h3>${escape(item.title)}</h3> </a>`;
      })}`;
    }
  })}</div> </main>`;
});
